...
static void slideshow_anim_ready_cb(lv_anim_t * a);
...
static void slideshow_anim_ready_cb(lv_anim_t * a)
{
    ...
    /* Plain, instant tab switch: on this MCU a mid-transition slide can't be
       rendered fast enough (near full-screen redraw) and shows up as a brief
       "page-flip" glitch instead of a smooth slide. A direct cut avoids that
       artifact entirely and matches the plain switch feel requested. */
    lv_tabview_set_act(tv, tab_id, LV_ANIM_OFF);
    ...
    lv_anim_t a_new;
    lv_anim_init(&a_new);
    lv_anim_set_exec_cb(&a_new, scroll_anim_y_cb);
    lv_anim_set_time(&a_new, t);
    lv_anim_set_playback_time(&a_new, t);
    lv_anim_set_values(&a_new, 0, v);
    lv_anim_set_var(&a_new, tab);
    lv_anim_set_ready_cb(&a_new, slideshow_anim_ready_cb);
    lv_anim_start(&a_new);
}
